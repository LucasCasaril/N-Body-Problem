...
R5_0 = [2000, 0, 0] # Initial Position of the Fifth Body (km)

V1_0 = [0, 0, 10] # Initial Velocity of the First Body (km/s)
V2_0 = [0, 30, 10] # Initial Velocity of the Second Body (km/s)
V3_0 = [0, 20, 15] # Initial Velocity of the Third Body (km/s)
V4_0 = [0, 60, 10] # Initial Velocity of the Fourth Body (km/s)
V5_0 = [0, 60, 10] # Initial Velocity of the Fifth Body (km/s)

# Initial Condition - Vector (24x1)
...
#y_result = rkf_integration(dydt, t0, tf, y0, G, m1, m2, m3, m4, tol, h)
#y_result = rkf_integration(dydt, t0, tf, y0, G, m, tol, h)
...
for i in range(len(X1)):
    ...
    XG[i] = ((m1*X1[i] + m2*X2[i] + m3*X3[i] + m4*X4[i])/(m1 + m2 + m3 + m4))
    YG[i] = ((m1*Y1[i] + m2*Y2[i] + m3*Y3[i] + m4*Y4[i])/(m1 + m2 + m3 + m4))
    ZG[i] = ((m1*Z1[i] + m2*Z2[i] + m3*Z3[i] + m4*Z4[i])/(m1 + m2 + m3 + m4))
